...
variable "versioning_mfa" {
  ...
  # TODO: 条件を見直し
  # validation {
  #   condition     = var.versioning_mfa == null && var.enable_versioning_mfa_delete == true ? can(length(var.versioning_mfa) > 0) : true
...
variable "lifecycle_filter_prefix" {
  description = "The prefix to apply to the lifecycle"
  type        = string
  nullable    = true
  default     = null
}

variable "lifecycle_rule_transition_to_standard_ia_days" {
  description = "The number of days to wait before transitioning to Standard-IA storage class"
  type        = number
  nullable    = true
  default     = 30
}

variable "lifecycle_rule_transition_to_glacier_days" {
  description = "The number of days to wait before transitioning to Glacier storage class"
  type        = number
  nullable    = true
  default     = 90
...
variable "lifecycle_rule_expire_days" {
  description = "The number of days to wait before expiring the object"
  type        = number
  nullable    = true
  default     = 395 # 365 + 30 days

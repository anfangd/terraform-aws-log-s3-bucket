...
resource "aws_s3_bucket_versioning" "this" {
  ...
  versioning_configuration {
    ...
    mfa_delete = try(var.enable_versioning_mfa_delete, false) ? "Enabled" : "Disabled"
  }
}
...
resource "aws_s3_bucket_lifecycle_configuration" "this" {
  ...
  rule {
    ...
    filter {
      prefix = var.lifecycle_filter_prefix == null ? "" : var.lifecycle_filter_prefix
    }

    transition {
      days          = var.lifecycle_rule_transition_to_standard_ia_days
      storage_class = "STANDARD_IA"
    }

    transition {
      days          = var.lifecycle_rule_transition_to_glacier_days
      storage_class = "GLACIER"
    }

    expiration {
      days = var.lifecycle_rule_expire_days
    }

    abort_incomplete_multipart_upload {
      days_after_initiation = 7 # days
    }
  }
}
